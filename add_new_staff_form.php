...
<hr class="seperator">
			<div class="form-group  row justify-content-md-center  "><!--name-->
			  ...
			  <div class="col-3">
				<input class="form-control noerror" type="text" placeholder="first name" name="first_name" id="first_name" onkeypress="return isalphabetonly(event)"  onkeyup="javascript:capitalize(this.id, this.value);">
			  </div>
			  <div class="col-3">
				<input class="form-control noerror" type="text" placeholder="last name" name="last_name" id="last_name" onkeypress="return isalphabetonly(event)"  onkeyup="javascript:capitalize(this.id, this.value);">
			  </div>
			  <div class="col-2">
			  </div>
			</div>
			<div class="form-group  row justify-content-md-center  "><!--name-->
			  ...
			  <div class="col-3">
				...
				<input class="form-control" type="text" placeholder="user name" name="username" value="" id="username" >
			  </div>
			...
			<label class="col-md-2 col-form-label required" for="sex-input"> Sex </label>
				<div id="sex-input" class="form-input col-md-3 noerror">
					<select name="sex" class="form-control noError" id="sel1">
			...
			<div class="form-group  row justify-content-md-center "><!--Date of birth--><!--Contact no-->
			  ...
			  <div class="col-3 ">
				<input name="dob" class="form-control noerror" type="date" data-date-format="DD MM YYYY" value="" placeholder="Age" id="age-input">
			  </div>
			  ...
			  <div id="designation-input" class="form-input col-3 noerror">
					<select name="designation" class="form-control" id="designation" oninput="fill_hidden(this.options[this.selectedIndex].getAttribute('data-department_id'),this.options[this.selectedIndex].getAttribute('data-department_name'))" >
...
function validateForm() {
	...
	var bloodgroup = document.forms["user_form"]["bloodgroup"].value;
    if (fname == "" ) {
		...
		swalError("Email cannot be empty");//alert
		$("#email-input").focus();
		$("#email-input").addClass('error').removeClass('noerror');
        return false;
       ...
       swalError("address must be filled out");//alert
		$("#address").focus();
		$("#address").addClass('error').removeClass('noerror');
        return false;
    }else if (bloodgroup == "" ) {
       swalError("bloodgroup must be selected");//alert
		$("#bloodgroup-input-select").focus();
		$("#bloodgroup-input-select").addClass('error').removeClass('noerror');
        return false;
    }else if (ICE_contact == "" ) {
       swalError("ICE contact must be filled out");//alert
        ...
        return false;
    }else if (ICE_contact.length < 10 ) {
       swalError("ICE contact must be 10 Digits");//alert
		$("#ICE-tel-input").focus();
		$("#ICE-tel-input").addClass('error').removeClass('noerror');
        return false;
    }else if (ICE_address == "" ) {
...
/*check username*/
    ...
    document.getElementById("username").oninput = function (e){
     ...
     document.getElementById("tel-input-staff").oninput = function (e){
        var user_name = $(this).val();
		//alert(user_name);
        clearTimeout(x_timer);
        x_timer = setTimeout(function(){
      ...
      $("#user-result").html(data);
		if (data=="1"){
			console.log(`the username used is ${username}`);
				...
				console.log(`the username used is  2 ${username}`);
