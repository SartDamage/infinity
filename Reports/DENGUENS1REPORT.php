...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">DENGUE NS1 ANTIGEN TEST</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">Dengue NS1 Antigen</td>
                    <td class="width-20 text-center">
                        <!--<input name="ctl00_reportmaster_txtDengueNS1" type="text" maxlength="100" id="ctl00_reportmaster_txtDengueNS1" class="txtbox-css width-90px margin-left-97" />-->
						<select name="ctl00_reportmaster_txtDengueNS1" id="ctl00_reportmaster_txtDengueNS1"  class="txtbox-css width-90px margin-left-97">
        ...
        <div class="hmms-rptnote">
            <span>Method : Qualitative determination by Rapid immunochromatpography assay<br><br />
			Note:<br>
			1. Dengue NS1 Antigen is a highly conserved glycoprotien that is present at high concentartion in the sera of dengue infected <br>
			Patient during early clinical phase of the disease
			2. NS1 antigen is found from the first day and upto 9 days after the onset od illness in case of primary and secondary dengue <be>infected patients
			3. Positive test indicates exposure to and infection with Dengue virus 
			4. Negative result can occure if the quentity of NS1 antigen present in the specimen is below the detection limit of the assay
               </span>
...
function validateForm() {
    var dng = document.forms["aspnetForm"]["ctl00_reportmaster_txtDengueNS1"].value;
    if (dng == "" || dng == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtDengueNS1").focus();
		$("#ctl00_reportmaster_txtDengueNS1").addClass('error').removeClass('noerror');
        return false;	
    }else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtDengueNS1', jsonforindividual.Dengue_NS1_Antigen);
}
	else{}
}  	
