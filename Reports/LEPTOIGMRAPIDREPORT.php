...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    <form method="post" action="./LeptoIgmRapidReport.aspx" id="aspnetForm" class="section-to-print">
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">LEPTO IGM RAPID</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">Leptospira - IgG</td>
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtLeptoIgg" type="text" maxlength="100" id="ctl00_reportmaster_txtLeptoIgg" class="txtbox-css width-90px margin-left-97" /></td>
                    <td class="width-40">
                    </td>
                </tr>
               </table>
        ...
        <div class="hmms-rptnote">
            <span>METHOD - IMMUNOCHROMATOGRAPHY<br />
 Material Used - J.Mitra (Card Test)<br /></span>
...
$.ajax({
	...
	$.ajax({
	   type: "GET",
	   url: "/set_p_LeptolgmRapidReport.php",
...
function validateForm() {
    var a = document.forms["aspnetForm"]["ctl00_reportmaster_txtLeptoIgg"].value;
    if (a == "" || a == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtLeptoIgg").focus();
		$("#ctl00_reportmaster_txtLeptoIgg").addClass('error').removeClass('noerror');
        return false;	
    }else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtLeptoIgg', jsonforindividual.Leptospira_IgM);
}
	else{}
}  	
