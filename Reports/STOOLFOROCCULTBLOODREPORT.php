...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">STOOL FOR OCCULT BLOOD</div>
        </div>
            ...
            <table class="width-100">
                ...
                <tr>
                    ...
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtOccult" type="text" maxlength="100" id="ctl00_reportmaster_txtOccult" class="txtbox-css width-90px margin-left-97" /></td>
...
$( "form#aspnetForm" ).on( "submit", function(event) {
	...
	$.ajax({
	   type: "GET",
	   url: "/set_p_StoolOccultBloodReport.php",
...
function validateForm() {
    var Occult = document.forms["aspnetForm"]["ctl00_reportmaster_txtOccult"].value;
    if (Occult == "" || Occult == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtOccult").focus();
		$("#ctl00_reportmaster_txtOccult").addClass('error').removeClass('noerror');
        return false;	
    }else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtOccult', jsonforindividual.Occult_Blood);
}
	else{}
}  	
