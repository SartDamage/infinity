...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">SR. URIC ACID</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">Uric Acid</td>
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtUric_acid" type="text" maxlength="100" id="ctl00_reportmaster_txtUric_acid" class="txtbox-css width-90px margin-left-97" /></td>
                    <td class="width-40">Males : 2.0 - 7.0 mg / dl Females: 2.0 - 6.5 mg/dl
                    </td>
                </tr>
...
$.ajax({
	...
	$.ajax({
	   type: "GET",
	   url: "/set_p_SrUricAcidReport.php",
...
function validateForm() {
    var a = document.forms["aspnetForm"]["ctl00_reportmaster_txtUric_acid"].value;
    if(a == "" || a == null){
		alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtUric_acid").focus();
		$("#ctl00_reportmaster_txtUric_acid").addClass('error').removeClass('noerror');
	}else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtUric_acid', jsonforindividual.Uric_acid);
		}
}  	
	</script>
