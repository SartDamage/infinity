...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">"TROPONIN T"</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">TROPONIN T</td>
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtTROPONIN" type="text" maxlength="100" id="ctl00_reportmaster_txtTROPONIN" class="txtbox-css width-90px margin-left-97" /></td>
...
function validateForm() {
    var cd = document.forms["aspnetForm"]["ctl00_reportmaster_txtTROPONIN"].value;
    if (cd == "" || cd == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtTROPONIN").focus();
		$("#ctl00_reportmaster_txtTROPONIN").addClass('error').removeClass('noerror');
        return false;	
    }else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtTROPONIN', jsonforindividual.Troponin_T);
}
	else{}
}  	
