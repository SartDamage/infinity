<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">CBC, ESR and PLATELET</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    ...
                    <td class="width-20 text-center">
                        <input onkeypress="return isNumber(event)" name="ctl00_reportmaster_txtESR" type="text" maxlength="100" id="ctl00_reportmaster_txtESR" class="txtbox-css width-90px margin-left-97" /></td>
                    <td class="width-40">0 - 22 mm / hour for Male<br>0 - 29 mm / hour for Female.
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div class="hmms-click section-notto-print"><input onkeypress="return isNumber(event)" type="submit" name="ctl00_reportmaster_btnSave" value="Save" id="ctl00_reportmaster_btnSave" class="btn-css primry-colr" /><?php $db = getDB();  $statement=$db->prepare("SELECT  *  FROM `pathopatientregistrationmaster`  WHERE  `PatientId`=:PathoRegID  AND `payment`=0" );  $statement->bindParam(':PathoRegID', $ID, PDO::PARAM_INT); $statement->execute();  if ( $statement->rowCount() <= 0){ echo '<input onkeypress="return isNumber(event)" type="button"   name="ctl00_reportmaster_btnPrint" value="Print" id="ctl00_reportmaster_btnPrint" class="btn-css primry-colr pull-right" />';}?>
        </div>
    </div>
...
$.ajax({
	...
	$.ajax({
	   type: "GET",
	   url: "/set_p_CBCEsrWithPlateletReport.php",
...
setSelectValue('ctl00_reportmaster_txtESR', jsonforindividual.Esrendof1hour);
}
	else{}
}  	
