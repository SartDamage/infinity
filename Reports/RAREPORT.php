...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">EXAMINATION OF BLOOD (R.A. TEST)</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">R.A.TEST</td>
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtRATEST" type="text" maxlength="100" id="ctl00_reportmaster_txtRATEST" class="txtbox-css width-90px margin-left-97" /></td>
                    <td class="width-40">0 - 14 IU/ml</td>
                </tr>
                <tr>
...
function validateForm() {
    var TEST = document.forms["aspnetForm"]["ctl00_reportmaster_txtRATEST"].value;
    var TITRE = document.forms["aspnetForm"]["ctl00_reportmaster_txtTITRE"].value;
    if (TEST == "" || TEST == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtRATEST").focus();
		$("#ctl00_reportmaster_txtRATEST").addClass('error').removeClass('noerror');
        return false;	
    }else if (TITRE == "" || TITRE == null) {
        alert("Parameters must be filled must be filled out");
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtRATEST', jsonforindividual.RATest);
