...
<?php include $_SERVER['DOCUMENT_ROOT']."/include/include_in_patho_report.php"; ?><body>
    ...
    <div class="hmms_hdr">
        <div class="">
            <div class="hmms-reprtname">CD3/CD4/CD8</div>
        </div>
        <div class="profile_tbl">
            <table class="width-100">
                ...
                <tr>
                    <td class="width-40">CD3 CD4</td>
                    <td class="width-20 text-center">
                        <input name="ctl00_reportmaster_txtCD3" type="text" maxlength="100" id="ctl00_reportmaster_txtCD3" class="txtbox-css width-90px margin-left-97" /></td>
        ...
        <div class="hmms-click section-notto-print"><input type="submit" name="ctl00_reportmaster_btnSave" value="Save" id="ctl00_reportmaster_btnSave" class="btn-css primry-colr" />
		<?php 
		$db = getDB(); 
		$statement=$db->prepare("SELECT * FROM `pathopatientregistrationmaster` WHERE `PatientId`=:PathoRegID AND `payment`=0");
		$statement->bindParam(':PathoRegID', $ID, PDO::PARAM_INT);
		$statement->execute();
		if ($statement->rowCount() <= 0){ echo '<input type="button" onclick="printreport(\'aspnetForm\')" name="ctl00_reportmaster_btnPrint" value="Print" id="ctl00_reportmaster_btnPrint" class="btn-css primry-colr pull-right" />';}else{ }?>
        </div>
    </div>
...
function validateForm() {
    var cd = document.forms["aspnetForm"]["ctl00_reportmaster_txtCD3"].value;
    if (cd == "" || cd == null) {
        alert("Parameters must be filled must be filled out");
		$("#ctl00_reportmaster_txtCD3").focus();
		$("#ctl00_reportmaster_txtCD3").addClass('error').removeClass('noerror');
        return false;	
    }else{ return true;}
}
...
function parseJsonToFormIndividual(jsonforindividual){
	...
	if (jsonforindividual){
		...
		setSelectValue('ctl00_reportmaster_txtCD3', jsonforindividual.CD3_CD4);
}
	else{}
}  		
</script>
