...
<?php
              ...
              $db=null;
              $db = getDB();
              $statement=$db->prepare("SELECT count(`id`) FROM add_vt_details where YEAR(WhenEntered)=YEAR(CURDATE())");
              $statement->execute();
              $results=$statement->fetchColumn();
              $json=json_encode($results);
              //return $json;
              ...
              $db = getDB();
              $statement=$db->prepare("SELECT count(`id`) FROM add_vt_details where MONTH(WhenEntered)=MONTH(CURDATE()) and YEAR(WhenEntered)=YEAR(CURDATE())");
              $statement->execute();
    ...
    <div class="card-block" id="print">
      ...
      <form class="form " name="tl_submit_form"  id="tl_submit_form" >
        ...
        <div class="form-group row justify-content-center"><!--name-->
          ...
          <div class="col-8">
          <input class="form-control noerror" type="text" tabindex="-1" placeholder="Name" name="tl_patient_name" value="" id="tl_patient_name"  >
          </div>
        </div>
        ...
        <div class="form-group row justify-content-center">
         <label for="tl_date" class="col-2 col-form-label required">Date of TL</label>
                                    ...
                                    <div id="tl_date_div" class="col-3 input-group date">
                                      <input class="form-control" type="text" id="tl_date" name="tl_date" oninput="myFunction()" autocomplete="off" />
                                      <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
                                </div>
          <label for="tl_monthly_income" class="col-2 col-form-label required">Monthly Income</label>
          <div class="col-3">
            <input type="number"class="form-control" id="tl_monthly_income"  placeholder="Enter the Monthly Income" name="tl_monthly_income" autocomplete="off" style="width: 100%; resize: none;"/>
      ...
      <div class="row justify-content-center">
        <div class="col-md-2"><br>
          <input type="submit" class="btn btn-success " name="tl_details_submit" id="tl_details_submit" value="Submit" style="width:150px; "/>
        </div>
        <!--<div class="col-6">
        <center>
...
if (test1 == "" ){
    swalError("regID must be filled out");//alert
$("#regId").focus();
...
$( "form#tl_submit_form" ).on( "submit", function( event ) {
  ...
  //alert("hello world");
    var url = "addpatient_tl.php"; // the script where you handle the form input.
  //validateForm(event)
      ...
      $.ajax({
           ...
           success: function(data)
           {
            ...
            if(data!="unsuccesful" && data !="" ){
              ...
              swalSuccess("Tl record Successfully added");//alert
...
<?php
$pageTitle = "Add TL Details"; // Call this in your pages' files to define the page title
