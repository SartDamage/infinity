...
<?php include $_SERVER['DOCUMENT_ROOT'].'/nav_bartop.php';?>
	<div class="container" id="test-form-container" style="padding-left:50px;margin-top:15px;">
		...
		<div class="row justify-content-md-center">
			<div class="col-md-4"><!--Pharmacy-->
				<div class="card mb-3 violet pointer" style="/* width: 18rem; */">
				<a class="reset-this" id="addp_stock"  data-toggle="tooltip" data-placement="bottom" title="Number of days present" >
				  <div class="card-body">
					...
					<div class="row">
						...
						<div class="col-8">
							<span class="important" id="available_bed" data-toggle="tooltip" data-placement="bottom" title="Days Present">
						 <?php
						 $cval="";
...
/* 	echo "<br>--------------------------------------------------------<br>";
	...
	$outtimearr=$normval->out_time_att;
			...
			<div class="col-md-4"><!--Pharmacy Stock-->
				...
				<a class="reset-this" id="ipd_bed"  data-toggle="tooltip" data-placement="bottom" title="Number of days absent" >
				  <div class="card-body">
					...
					<div class="row">
						...
						<?php
						$usernameid=$userDetails->user_id;
							$db = getDB();
	$statement=$db->prepare("SELECT * from attendance_manager where user_id=:category order by att_time ");
	$statement->bindParam(':category',$usernameid, PDO::PARAM_STR);
	$statement->execute();
	$results=$statement->fetchAll(PDO::FETCH_ASSOC);
...
/* 	echo "<br>--------------------------------------------------------<br>";
	...
	foreach($main_attendence_id_array as $user_id_from_main){
		...
		foreach($super_main_attendence_id_array as $normval)
		{       
	            $dateinarr1= $normval->date_att;
	$timearr1=$normval->in_time_att;
	$outtimearr1=$normval->out_time_att;
				if($timearr1==false or $outtimearr1==false)
				{
			array_push($absentcount,$dateinarr1);
		   ...
		   $abcountreal= sizeof($absentcount);
		   $abcountreal=$abcountreal-1;
		   if($abcountreal==-1)
		   {
	   $abcountreal=0;
	   echo $abcountreal;
		...
		//echo "<br>@@@@@@@@@@@@@@@@<br>@@@@@@@@@@@@@@@@<br>";
							...
							<span class="less_important" id="total_beds" data-toggle="tooltip" data-placement="bottom" title="Number of paid leave available">
								...
								$category = "Pharmacy";
								$db = getDB();
								$stmt = $db->prepare("SELECT * FROM stock_individual AS SI INNER JOIN stock_category AS SC ON SI.category=SC.ID WHERE SC.category=:category ");
			...
			<div class="col-md-4"><!--Pharmacy Stock-->
				...
				<a class="reset-this" id="phar_invoice"  data-toggle="tooltip" data-placement="bottom" title="Number of leaves available" >
				  <div class="card-body">
            ...
            <div id="my-calendar"></div>
                ...
                $(document).ready(function () {
                    $("#my-calendar").zabuto_calendar({
                        ...
                        ajax: {
                            url: "show_datacopy.php?grade="+userglobal+"&userid="+userglobal+"&action="+userglobal,
							      modal: true
                        }
                    });
                });
