...
/* 	echo "<br>--------------------------------------------------------<br>";
	...
	foreach($main_attendence_id_array as $user_id_from_main){
		...
		foreach($super_main_attendence_id_array as $monthsortloop)
		{
		 ...
		 {
				$intimefort=$monthsortloop->in_time_att;
				$outtimefort=$monthsortloop->out_time_att;
				$diffin=strtotime($intimefort);
				$diffout=strtotime($outtimefort);
				$monthdiff=$diffout-$diffin;
				//$subdate1= date('H:i:s', $monthdiff);
				$subdatehours= date('H', $monthdiff);
				 $subdatemin= date('i', $monthdiff);
				 $subdatefin=(int)($subdatehours*60)+$subdatemin;
				// echo $userinloopms." ".$subdatefin."||";
			 ...
			 $presentcountt++;
					 ...
					 {
				 if($monthsortloop->in_time_att == 0)
				 {
		...
		/* 	echo $intimefort;	
			...
			echo $outtimefort;
			echo "<br>"; */
			$attcountperuser->present =$presentcountt;
		    $attcountperuser->absent = $absentcount;
		    $attcountperuser->halfday=$halfdaycount;
			$attcountperuser->work="0";
			 }
