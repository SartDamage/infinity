<?php
include $_SERVER['DOCUMENT_ROOT'].'/include/conection.php';
include $_SERVER['DOCUMENT_ROOT'].'/session.php';
$userDetails=$userClass->userDetails($session_id);


	$form = $_POST;

	 //////for profile image Aj/////////////////////////////////
	 $target_dir = "upload/patient_img/";

	if(isset($form['fileToUpload'])){/* $encoded_data = $form['fileToUpload']; */
		$encoded_data ="";}else{$encoded_data =  ""
			;};
	$binary_data = base64_decode( $encoded_data );
	$uploadOk = 1;
	$name = date('YmdHis').".jpg";
	$target_file = $target_dir . $name;
	$newfilename = $name;
	if ($uploadOk == 0) {
		$errmsg = "Sorry, your file was not uploaded.";
	} else
	{
		file_put_contents($target_dir.$newfilename,$binary_data);
	}
/////////////////////////////////////////////////////////////////////////////

///////////////////////////////document of patient////////////////////////////
$target_dir1 ="upload/patient_doc/";

if(isset($form['fileToUpload1'])){$encoded_data1 = $form['fileToUpload1'];}
$data = explode( ',', $encoded_data1 );
$binary_data1 = base64_decode($data[ 1 ] );
  $name1 = date('YmdHis').".jpg";
	$target_file1 = $target_dir1 . $name;
	$newfilename1 = $name1;
  $uploadOk = 1;
	if ($uploadOk == 0) {
		$errmsg = "Sorry, your file was not uploaded.";
		} else {
			if (file_put_contents($target_dir1.$newfilename1,$binary_data1)) {
/////////////////////////////////////////////////////////////////////////////
				$fname=$form['first_name'];
				$mname=$form['middle_name'];
				$lname=$form['last_name'];
				//$patient_type=$form['patient_type'];
				$email=$form['email'];
				$contact=$form['contact'];
				$gender=$form['sex'];
				$marital_status=$form['marital_status'];
				$age=$form['age'];
				$alt_contact=$form['alt_contact'];
				$height=$form['height'];
				$weight=$form['weight'];
				$address=$form['address'];
				$bloodgroup=$form['blood_group'];
				$ICE_name=$form['ICE_name'];
				$ICE_relation=$form['ICE_relation'];
				$ICE_contact=$form['ICE_contact'];
				$avatar=$newfilename;
				$avatar1=$newfilename1;
				if(isset($form['GI_type'])){

					$GI_type=$form['GI_type'];
					$GI_type= base64_encode($GI_type);
				}else{
					$GI_type="";
				}
				$GI_number=$form['GI_number'];
				$GI_number=base64_encode($GI_number);
				/* $ward_no=$form['ward_no'];
				$bed_no=$form['bed_no'];
				$amount_deposit=$form['amount_deposit']; */
				/* make same if checked */
				$ICE_address=$form['ICE_address'];
				/*  */
				$med_history=$form['med_history'];
				$history=$form['med_history_detail'];
				$AdminID=$form['AdminID'];


	$email_check = preg_match('~^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.([a-zA-Z]{2,4})$~i', $email);
	$contact = preg_replace("/[^0-9]/", '', $contact);
	$alt_contact = preg_replace("/[^0-9]/", '', $alt_contact);
	$ICE_contact = preg_replace("/[^0-9]/", '', $ICE_contact);

	if($gender==null || $gender == "" || $fname==null || $fname == "" || $contact==null||$contact=="" || $address==null||$address=="" ||  $ICE_contact==null || $ICE_contact==""){
			//header("Location: ./addpatientform.php"); /* Redirect browser */
	}else{
		$patexists=$userClass->checkpatient($fname,$lname,$contact,$gender);
		if($patexists==true){
			echo "No Patients added, already exists";
			//header("Location: ./addpatientform.php");
		}else{
			$reg_patient=$userClass->addPatient($fname,$mname,$lname,$email,$alt_contact,$gender,$marital_status,$age,$contact,$height,$weight,$address,$avatar,$avatar1,$bloodgroup,$ICE_name,$ICE_relation,$ICE_contact,$ICE_address,$med_history,$history,$AdminID,$GI_type,$GI_number);//,$history);
			if($reg_patient){
				echo $reg_patient;
				//header("Location: ./addpatientform.php"); /* Redirect browser */
				exit();
				}else{echo "No Patients added";}
			}

	}
			}else {
			$errmsg = "Sorry, there was an error uploading your file.";
			}
		}
/* 		else {
			$errmsg = "Sorry, there was an error uploading your file.";
		} */
//else{/*echo'<script>alert("not subbmitted not added")</script>';*/}
/*
if (!empty($_POST['patetient_registration']))
{
	$form = $_POST;
	$fname=$form['first_name'];
	$lname=$form['last_name'];
	//$patient_type=$form['patient_type'];
	$email=$form['email'];
	$contact=$form['contact'];
	$gender=$form['sex'];
	$marital_status=$form['marital_status'];
	$age=$form['age'];
	$alt_contact=$form['alt_contact'];
	$height=$form['height'];
	$weight=$form['weight'];
	$address=$form['address'];
	$bloodgroup=$form['blood_group'];
	$ICE_name=$form['ICE_name'];
	$ICE_relation=$form['ICE_relation'];
	$ICE_contact=$form['ICE_contact'];
	/* $ward_no=$form['ward_no'];
	$bed_no=$form['bed_no'];
	$amount_deposit=$form['amount_deposit']; */
	/* make same if checked */
/*	$ICE_address=$form['ICE_address'];
	/*  */
/*	$med_history=$form['med_history'];
	$history=$form['med_history_detail'];

	$email_check = preg_match('~^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.([a-zA-Z]{2,4})$~i', $email);
	$contact = preg_replace("/[^0-9]/", '', $contact);
	$alt_contact = preg_replace("/[^0-9]/", '', $alt_contact);
	$ICE_contact = preg_replace("/[^0-9]/", '', $ICE_contact);

	if($gender==null || $gender == "" || $fname==null || $fname == "" || $contact==null||$contact=="" || $address==null||$address=="" || $patient_type==null||$patient_type=="" || $ICE_contact==null || $ICE_contact==""){

			echo '<script>alert("Please Fill All Required Field")</script>';
			header("Location: ./addpatientform.php"); /* Redirect browser */
/*			return 	false;
	}else{
		$patexists=$userClass->checkpatient($fname,$lname,$contact,$gender);
		if($patexists==true){
			echo "<script>alert('Patients already exists')</script>";
			header("Location: ./addpatientform.php");
			return 	false;
		}else{
			$reg_patient=$userClass->addPatient($fname,$lname,$patient_type,$email,$alt_contact,$gender,$marital_status,$age,$contact,$height,$weight,$address,$bloodgroup,$ICE_name,$ICE_relation,$ICE_contact,$ICE_address,$med_history,$history);//,$history);
			if($reg_patient){
			echo "<script>alert('Patients added Successfully')</script>";
				header("Location: ./addpatientform.php"); /* Redirect browser */
/*				exit();
			}
		}

	}

}	*/
/*
$db = getDB();
$statement=$db->prepare("SELECT * FROM patientregistrationmaster");
$statement->execute();
$results=$statement->fetchAll(PDO::FETCH_ASSOC);
$json=json_encode($results);
//echo $json; */
$db=null;
?>
